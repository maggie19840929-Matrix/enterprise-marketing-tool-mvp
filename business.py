...
def recommend_platforms_for_assessment(assessment):
    ...
    if _has_any(text, ['口腔', '牙', '门诊', '种植', '矫正', '正畸', '宝妈']):
    ...
    elif _has_any(text, ['美业', '美甲', '美睫', '美容', '皮肤管理', '医美', '产康', '纹眉', '半永久']):
        add(primary, '小红书', '适合承接同城搜索、效果案例、避坑清单和价格顾虑。')
        add(primary, '抖音', '适合用短视频放大同城曝光，展示服务过程、环境和前后变化。')
        add(primary, '朋友圈/私域', '适合老客复购、转介绍、活动提醒和信任维护。')
        add(support, '美团/大众点评', '适合承接到店意图，重点优化套餐、评价和门店页转化。')
        add(support, '视频号', '适合微信生态内做案例沉淀和熟人信任。')
        add(avoid, 'B站', '本地到店转化链路较长，不建议作为第一主阵地。')
    elif _has_any(text, ['餐饮', '饭店', '餐厅', '咖啡', '茶饮', '火锅', '烧烤', '烘焙', '甜品', '小吃']):
        add(primary, '抖音', '适合用同城短视频放大菜品、环境、活动和到店氛围。')
        add(primary, '小红书', '适合做同城探店、收藏清单、场景种草和菜单决策内容。')
        add(primary, '美团/大众点评', '适合承接搜索、评价、团购和到店转化。')
        add(support, '朋友圈/私域', '适合老客复购、会员活动和社群触达。')
        add(support, '视频号', '适合老板/门店日常和微信生态活动承接。')
        add(avoid, 'B站', '短期到店效率低，除非已有长视频内容能力。')
    elif _has_any(text, ['教育', '培训', '课程', '报名', '留学', '考试', '教培', '托管', '素质教育']):
        add(primary, '小红书', '适合用学习经验、避坑、家长疑问和案例内容承接主动搜索。')
        add(primary, '视频号', '适合家长/熟人圈层转化、直播讲解和信任沉淀。')
        add(primary, '朋友圈/私域', '适合跟进试听、答疑、转介绍和报名转化。')
        add(support, '抖音', '适合扩大曝光，但需要高频短视频和强钩子。')
        add(avoid, 'B站', '适合长期知识资产，不适合短期报名转化主渠道。')
    elif _has_any(text, ['本地', '到店', '门店', '附近', '同城', '本地生活', '家政', '维修', '摄影', '宠物', '体验课']):
        ...
        add(support, '朋友圈/私域', '适合承接信任、复购和轻咨询转化。')
        add(support, '视频号', '适合沉淀微信生态信任和私域承接。')
        add(support, '美团/大众点评', '如果属于到店服务，可作为搜索评价和转化承接平台。')
...
def _plan_templates(priority, industry, goal, target, offer, pain):
    ...
    common = [
        ...
        (f'一个真实场景：{target}如何判断是否需要{offer}', '案例信任：前后变化/过程/结果', '短视频', f'通过主页咨询「{offer}」是否适合自己', '私信数'),
        ...
        (f'{industry}负责人亲自说：我们如何服务{target}', '人设信任：真实、专业、有温度', '短视频', f'有「{pain}」类似问题可以私信具体情况', '互动数'),
        (f'本周{target}问得最多的1个问题', 'FAQ：把咨询问题反向变成内容', '短视频/图文', f'还有关于「{offer}」的问题，可以通过主页咨询', '评论数'),
    ]
    if priority == '曝光不足':
