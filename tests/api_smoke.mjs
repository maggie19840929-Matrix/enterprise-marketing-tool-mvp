...
const INTERNAL_ACCESS_TOKEN = 'smoke-internal-token-1.6.96';
const FEISHU_INBOUND_TOKEN = 'smoke-feishu-inbound-token-1.6.96';
process.env.INTERNAL_ACCESS_TOKEN = INTERNAL_ACCESS_TOKEN;
process.env.METERING_HASH_SECRET = 'smoke-metering-secret-v1.6.96-not-production';
...
assert(diagnosis.app_version === '1.6.96', `expected app_version 1.6.96, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.96'") && appJs.includes("v1.6.96 · 品牌叙事完善版"), 'app should expose the v1.6.96 brand narrative release internally/API-side');
...
assert(indexHtml.includes('/app.js?v=1.6.96') && indexHtml.includes('/styles.css?v=1.6.96') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.96'), 'customer page should cache-bust the v1.6.96 brand narrative release while preserving the first-paint fix');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.96') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the FP Matrix elephant lockup and first-party growth descriptor');
...
assert([aboutHtml, privacyHtml, termsHtml, contactHtml].every((html) => html.includes('fp-matrix-lockup') && html.includes('企业第一方增长智能') && html.includes('/fp-matrix-elephant.svg?v=1.6.96')), 'all independent customer information pages should use the same FP Matrix lockup');
...
assert(health.version === '1.6.96' && health.version_label === 'v1.6.96 · 品牌叙事完善版', 'health should expose the v1.6.96 brand narrative release');
