...
assert(diagnosis.app_version === '1.6.171', `public diagnosis should return app_version 1.6.171, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.171'") && appJs.includes("v1.6.171 · 图文素材包顺序加载版"), 'application should expose the reviewed v1.6.171 sequential image material pack release');
...
assert(appJs.includes('function generationMaterialPackPreview') && appJs.includes('const generationMediaLoadPromises = new Map()') && appJs.includes('function generationMediaNodesForAsset') && appJs.includes('async function revealGenerationMedia') && appJs.includes('for (const assetId of assetIds)') && appJs.includes('window.setTimeout(resolve, 1200)') && warRoomCss.includes('.generation-material-thumb') && warRoomCss.includes('aspect-ratio:2/3'), 'completed material-pack cards should deduplicate and sequentially retry large media loads before revealing responsive previews');
...
assert(indexHtml.includes('/app.js?v=1.6.171') && indexHtml.includes('/styles.css?v=1.6.171') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.171'), 'public customer page should use the v1.6.171 cache-busted asset references');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.171') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.171') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.171') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert(health.version === '1.6.171' && health.version_label === 'v1.6.171 · 图文素材包顺序加载版', 'public application health version should report v1.6.171');
