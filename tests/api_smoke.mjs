...
assert(diagnosis.app_version === '1.6.175', `public diagnosis should return app_version 1.6.175, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.175'") && appJs.includes("v1.6.175 · 小红书成稿质量门禁版"), 'application should expose the reviewed v1.6.175 Xiaohongshu copy quality-gate release');
...
assert(indexHtml.includes('/app.js?v=1.6.175') && indexHtml.includes('/styles.css?v=1.6.175') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.175'), 'public customer page should use the v1.6.175 cache-busted asset references');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.175') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.175') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.175') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert(health.version === '1.6.175' && health.version_label === 'v1.6.175 · 小红书成稿质量门禁版', 'public application health version should report v1.6.175');
...
assert(lastKimiUserPrompt.includes('完整成稿末尾必须单独给出“话题标签”') && lastKimiUserPrompt.includes('6至8个带#号的相关标签'), 'Kimi should receive a complete Xiaohongshu hashtag requirement instead of omitting publish metadata');
...
kimiResponseQueue = [
  ...
  {
    text: '标题：老板别每天现想\n每次发布前先排好一周选题，再按天整理素材。发布后记录数据，下一轮只保留真正有效的方向。\n话题标签：#内容营销 #小红书运营 #企业获客 #内容选题 #内容规划 #中小企业营销',
    finish_reason: 'stop',
  },
];
...
assert(qualityTask.adapter_manifest?.output?.quality_repair_attempted === true && qualityTask.adapter_manifest?.output?.hashtag_count === 6, 'quality gate should record one automatic rewrite and the final hashtag count');
assert(!/Free版|主页有演示|立即咨询/.test(qualityAsset?.notes || '') && (qualityAsset?.notes.match(/#/g) || []).length === 6, 'automatic rewrite must remove early hard selling and deliver six relevant hashtags');
