...
assert(appJs.includes("const APP_VERSION = '1.6.176'") && appJs.includes("v1.6.176 · 小红书发布包排版优化版"), 'application should expose the reviewed v1.6.176 Xiaohongshu publish-package release');
...
assert(indexHtml.includes('id="internalAccessGate"') && indexHtml.includes('id="internalAccessForm"') && indexHtml.includes('id="internalAccessToken"') && indexHtml.includes('autocomplete="one-time-code" maxlength="64"') && appJs.includes("tokenInput.value = ''"), 'internal shell should accept secure access codes, avoid stale password autofill and clear rejected values before retrying');
...
assert(appJs.includes('function xiaohongshuPublishPackage') && appJs.includes('data-gw-action="copy-output-title"') && appJs.includes('data-gw-action="copy-output-body"') && appJs.includes('data-gw-action="copy-output-tags"') && appJs.includes('function copyGenerationTaskOutputPart'), 'Xiaohongshu publish packages should support separate title, body and precise-tag copying while old outputs retain whole-copy compatibility');
...
assert(indexHtml.includes('id="generationClientSelect"') && indexHtml.includes('id="generationProjectSelect"') && indexHtml.includes('id="generationScopeStatus"'), 'generation workbench should expose customer and project selection before the content-plan selector');
assert(appJs.includes('let generationScopeState =') && appJs.includes('function renderGenerationScope') && appJs.includes('function loadGenerationClientProjects') && appJs.includes("$('#generationClientSelect')?.addEventListener('change'") && appJs.includes("$('#generationProjectSelect')?.addEventListener('change'"), 'generation customer and project selectors should load cloud projects and react to operator changes');
assert(appJs.includes('const generationCustomerOptions =') && appJs.includes('Number(record.project_count || 0) > 0') && appJs.includes('const customers = generationCustomerOptions();'), 'generation workbench should collapse duplicate customer records and prefer a record that contains a usable project');
assert(indexHtml.includes('<option>朋友圈/私域</option>') && indexHtml.includes('<option>美团/大众点评</option>'), 'generation platform selector should preserve every platform used by customer content plans instead of collapsing them to other');
...
assert(indexHtml.includes('id="generationOpenAssetUpload"') && indexHtml.includes('id="generationAssetPanel"') && indexHtml.includes('id="generationAssetContext"') && appJs.includes("$('#generationOpenAssetUpload')?.addEventListener('click'") && appJs.includes("panel.querySelector('[name=\"asset_file\"]')?.focus()"), 'generation workbench should expose a direct reference-asset upload entry and focus the existing upload field');
assert(warRoomCss.includes('body.internal-mode .generation-field-action-head') && warRoomCss.includes('body.internal-mode .generation-asset-context'), 'reference-asset entry and project context should use scoped internal workbench styles');
assert(apiSource.includes('const persistInlineAssetContent =') && apiSource.includes('asset-files/${normalizeClientId(clientId)') && apiSource.includes("store.set(key, bytes") && apiSource.includes("path.match(/^\\/assets\\/([^/]+)\\/content$/)"), 'generated image bytes should be stored separately from lightweight asset metadata and exposed through an authenticated content route');
assert(apiSource.includes("format !== 'json'") && apiSource.includes("getWithMetadata(directKey, { type: 'stream' })") && apiSource.includes("format === 'json'") && apiSource.includes('Binary payloads must bypass customer text sanitization') && apiSource.includes('content_base64: Buffer.from(content.bytes).toString') && apiSource.includes('directStore.getWithMetadata(directKey') && apiSource.includes('new Response(new Uint8Array(content.bytes)') && apiSource.includes("'cache-control': 'private, no-store'") && appJs.includes('const apiBlob = async') && appJs.includes("cache: 'no-store'") && appJs.includes('function hydrateGenerationOutputMedia') && appJs.includes('data-generation-media-asset-id') && appJs.includes('asset.sha256 || asset.updated_at || APP_VERSION') && appJs.includes('await apiBlob(`/api/assets/') && appJs.includes('await node.decode()') && appJs.includes('placeholder.remove()') && appJs.includes('downloadGenerationTaskAsset'), 'generation task cards should stream deterministic asset blobs, remove completed loading placeholders and hydrate through an authenticated uncached binary transport');
assert(indexHtml.includes('id="generationMaterialPack"') && indexHtml.includes('本条图文素材包') && appJs.includes('GENERATION_MATERIAL_PACK_SLOTS') && appJs.includes('function renderGenerationMaterialPack') && appJs.includes('generateMaterialPackBodyImages') && appJs.includes('material-pack-v1:') && appJs.includes('generationStoredZip') && appJs.includes('downloadGenerationMaterialPack'), 'internal generation workbench should recommend one cover plus three body images, track the pack and support single or ZIP downloads');
assert(appJs.includes('品牌蓝为主色') && appJs.includes('不得每张随机换色') && appJs.includes('当前封面会作为后续正文图的视觉基线'), 'material pack prompts should preserve a coherent visual family across cover and body images');
assert(appJs.includes('function generationMaterialPackPreview') && appJs.includes('const generationMediaLoadPromises = new Map()') && appJs.includes('function generationMediaNodesForAsset') && appJs.includes('async function revealGenerationMedia') && appJs.includes('for (const assetId of assetIds)') && appJs.includes('window.setTimeout(resolve, 1200)') && appJs.includes("const cachedUrl = generationMediaObjectUrls.get(String(asset.asset_id || ''))") && appJs.includes('return cachedResponse.blob()') && warRoomCss.includes('.generation-material-thumb') && warRoomCss.includes('aspect-ratio:2/3'), 'completed material-pack cards should sequentially load large previews and reuse cached files for ZIP downloads');
assert(apiSource.includes("const outputFormat = 'jpeg'") && apiSource.includes('output_compression: 80') && apiSource.includes('data:${imageMimeType};base64'), 'real OpenAI cover generation should request a compressed browser-friendly image while preserving the requested dimensions');
assert(appJs.includes('const [tasks, clientTasks] = await Promise.all([') && appJs.includes("const assets = await api(`/api/assets?${query}`, {timeoutMs: 60000})") && appJs.includes('latestOutputReady'), 'task status should render before large asset previews and completion copy should wait until output metadata is readable');
...
assert(appJs.includes("title: '内容生产工作台'") && indexHtml.includes('<summary>参考素材与交付设置</summary>') && !indexHtml.includes('关联项目、参考素材与交付设置') && indexHtml.includes('参考素材（可选）') && indexHtml.includes('技术与飞书调试'), 'internal generation workbench should keep customer/project association visible and collapse only secondary operational panels');
...
assert(indexHtml.includes('name="content_plan_record_id" type="hidden" value=""') && !indexHtml.includes('value="qa_content_plan_001"'), 'generation forms should start without QA ids and receive the real selected project plan binding');
...
assert(indexHtml.includes('/app.js?v=1.6.176') && indexHtml.includes('/styles.css?v=1.6.176') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.176'), 'public customer page should use the v1.6.176 cache-busted asset references');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.176') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.176') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.176') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert(dentalContextTask.production_context?.plan?.sequence === 1 && dentalContextTask.production_context?.plan?.total === 7, 'production context must tell the model where the selected item sits in the weekly sequence');
...
assert(lastKimiRequestBody?.temperature === 0.6, 'Kimi K2.6 copy and script generation must use its only supported temperature value');
...
assert(lastKimiUserPrompt.includes('发布时优先搜索词') && lastKimiUserPrompt.includes('系统补充标签') && lastKimiUserPrompt.includes('平台当时推荐的话题'), 'Kimi should separate live platform topic selection from precise system tags');
assert(apiSource.includes('属于信任建立阶段') && apiSource.includes('不得出现“Free版”“套餐”“价格”“购买”“主页有演示”“立即咨询”') && apiSource.includes('不要突然转成产品广告'), 'early Xiaohongshu copy should build trust before any hard product conversion');
assert(apiSource.includes('xiaohongshuCopyQuality') && apiSource.includes('kimi_xiaohongshu_quality_gate_failed') && apiSource.includes('quality_repair_attempted'), 'Xiaohongshu copy should be validated and repaired before entering internal QA');
...
kimiResponseQueue = [
  {
    text: '标题：老板每天现想选题\n每次发布都临时决定，Free版工具主页有演示，立即咨询即可查看。',
    finish_reason: 'stop',
  },
  {
    text: '标题：老板别每天现想\n正文：\n📌 每次发布前先排好一周选题，再按天整理素材。\n\n✅ 发布后记录真实数据，下一轮只保留真正有效的方向。照着这份节奏先执行一周。\n发布时优先搜索词：内容选题、企业内容营销、小红书运营\n系统补充标签：#企业获客 #内容规划 #中小企业营销\n发布前检查：\n□ 若文字、图片均为原创，请按实际情况勾选原创声明\n□ 发布时输入优先搜索词，并选择平台实时推荐且内容高度相关的话题',
    finish_reason: 'stop',
  },
];
const kimiQualityTask = (await (await handler(internalRequest('POST', 'generation-tasks', {
  project_id: 'project-dental',
  client_id: 'dental',
  client_name: '社区口腔门诊',
  content_plan_record_id: String(dentalData.plans[0].id),
  ...
  content_type: '文案',
  generation_type: 'copy',
  prompt: '根据当前第一条选题生成一篇完整小红书笔记',
  output_spec: { format: '小红书笔记', client_visible: false },
}))).json()).task;
await handler(internalRequest('POST', `generation-tasks/${kimiQualityTask.task_id}/submit`, { client_id: 'dental' }));
const qualityBackgroundResponse = await backgroundGenerationHandler(new Request('http://localhost/.netlify/functions/generate-background', {
  method: 'POST',
  headers: {
    'content-type': 'application/json',
    'x-background-generation-token': BACKGROUND_GENERATION_TOKEN,
  },
  body: JSON.stringify({ client_id: 'dental', task_id: kimiQualityTask.task_id }),
}));
assert(qualityBackgroundResponse.status === 200, 'non-compliant Xiaohongshu copy should be repaired inside the background function');
const qualityTask = (await (await handler(internalRequest('POST', `generation-tasks/${kimiQualityTask.task_id}/poll`, { client_id: 'dental' }))).json()).task;
const qualityAssets = (await (await handler(internalRequest('GET', 'assets?client_id=dental&project_id=project-dental'))).json()).assets;
const qualityAsset = qualityAssets.find((asset) => asset.asset_id === qualityTask.output_asset_ids[0]);
assert(qualityTask.status === 'qa_pending' && qualityTask.fallback === false, 'repaired Xiaohongshu copy should enter internal QA normally');
assert(qualityTask.adapter_manifest?.output?.quality_checked === true && qualityTask.adapter_manifest?.output?.quality_passed === true, 'repaired copy should retain positive quality-gate evidence');
assert(qualityTask.adapter_manifest?.output?.quality_repair_attempted === true && qualityTask.adapter_manifest?.output?.hashtag_count === 3 && qualityTask.adapter_manifest?.output?.keyword_count === 3, 'quality gate should record one automatic rewrite plus precise platform keywords and system tags');
assert(qualityTask.adapter_manifest?.output?.emoji_count === 2 && qualityTask.adapter_manifest?.output?.paragraph_count === 2, 'quality gate should retain paste-ready paragraph and emoji evidence');
assert(!/Free版|主页有演示|立即咨询|\*\*/.test(qualityAsset?.notes || '') && /发布时优先搜索词：/.test(qualityAsset?.notes || '') && /若文字、图片均为原创/.test(qualityAsset?.notes || ''), 'automatic rewrite must remove hard selling and deliver the full Xiaohongshu publish checklist');
assert(kimiGenerationCalls === 3, 'Xiaohongshu quality smoke should add one initial call and one automatic rewrite');
...
assert(kimiGenerationCalls === 8, 'Kimi completeness smoke should include the Xiaohongshu quality rewrite plus continuation and regeneration calls');
...
assert(kimiGenerationCalls === 12, 'Kimi completeness smoke should include the quality rewrite and four fail-closed provider calls');
...
assert(generatedBackgroundCoverAsset?.storage_url === 'data:image/jpeg;base64,AAAAAP01MatrixPTEAAAA=', 'customer sanitizer must not alter embedded image data URLs');
...
assert(customerAvatarDetail.image?.status === 'ready' && customerAvatarDetail.image?.image?.url?.startsWith('data:image/jpeg;base64,'), 'completed customer avatar should expose a usable image preview and download URL');
