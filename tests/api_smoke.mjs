...
assert(diagnosis.app_version === '1.6.160', `public diagnosis should return app_version 1.6.160, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.160'") && appJs.includes("v1.6.160 · 封面预览与素材存储修复版"), 'application should expose the reviewed v1.6.160 cover preview release');
...
assert(apiSource.includes("format === 'json'") && apiSource.includes('Binary payloads must bypass customer text sanitization') && apiSource.includes('content_base64: Buffer.from(content.bytes).toString') && apiSource.includes("'cache-control': 'private, no-store'") && appJs.includes('sanitizeResponse = true') && appJs.includes('sanitizeResponse ? sanitizeCustomerPayload(data) : data') && appJs.includes('function hydrateGenerationOutputMedia') && appJs.includes('data-generation-media-asset-id') && appJs.includes('asset.sha256 || asset.updated_at || APP_VERSION') && appJs.includes('sanitizeResponse: false') && appJs.includes('window.atob(String(payload.content_base64') && appJs.includes('await node.decode()') && appJs.includes('downloadGenerationTaskAsset'), 'generation task cards should hydrate and download separately stored image output without applying text sanitization to authenticated binary data or blocking task-list rendering');
...
assert(indexHtml.includes('/app.js?v=1.6.160') && indexHtml.includes('/styles.css?v=1.6.160') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.160'), 'public customer page should use the v1.6.160 cache-busted asset references');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.160') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.160') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.160') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert(health.version === '1.6.160' && health.version_label === 'v1.6.160 · 封面预览与素材存储修复版', 'public application health version should report v1.6.160');
