...
assert(appJs.includes("const APP_VERSION = '1.6.147'") && appJs.includes("v1.6.147 · 旧匿名项目找回版"), 'application should expose the reviewed v1.6.147 anonymous-project recovery release');
assert(appJs.includes('customerResumeCandidateFromBrowser') && appJs.includes('enterpriseMarketingMvpProjects\\.([a-z0-9_-]+)\\.v1') && appJs.includes('safeStorage.setItem(CUSTOMER_SESSION_KEY, candidate.client_id)') && appJs.includes('clientState = normalizeState(saved)'), 'customer resume should recover a generated project from an older browser-scoped anonymous client id without rebuilding its state');
...
assert(health.version === '1.6.147' && health.version_label === 'v1.6.147 · 旧匿名项目找回版', 'public application health version should report v1.6.147');
