...
assert(appJs.includes("const APP_VERSION = '1.6.146'") && appJs.includes("v1.6.146 · 项目恢复与旧文案修复版"), 'application should expose the reviewed v1.6.146 resume and legacy-copy repair release');
...
assert(!appJs.includes('const writeHashStore') && appJs.includes('const clearLegacyStateHash') && appJs.includes('return writeWindowStore(store);') && appJs.includes("for (const name of ['localStorage', 'sessionStorage'])") && appJs.includes('for (const area of storageAreas())'), 'storage fallback must preserve legacy hash reads, fall through local/session storage safely, and no longer write whole project state into the browser address bar');
...
assert(health.version === '1.6.146' && health.version_label === 'v1.6.146 · 项目恢复与旧文案修复版', 'public application health version should report v1.6.146');
