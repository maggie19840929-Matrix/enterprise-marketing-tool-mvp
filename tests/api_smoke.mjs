...
assert(diagnosis.app_version === '1.6.146', `public diagnosis should return app_version 1.6.146, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.146'") && appJs.includes("v1.6.146 · 客户云存储恢复版"), 'application should expose the reviewed v1.6.146 customer cloud storage recovery release');
...
assert(apiSourceIncludes('getStore(CLOUD_STATE_STORE)') && apiSourceIncludes('runtimeContext?.uncachedEdgeURL') && apiSourceIncludes('await explicitStore.get'), 'cloud storage should prefer the deployment-scoped runtime context without requiring strong consistency and verify any explicit fallback credentials');
...
assert(indexHtml.includes('/app.js?v=1.6.146') && indexHtml.includes('/styles.css?v=1.6.146') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.146'), 'public customer page should use the v1.6.146 cache-busted asset references');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.146') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.146') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.146') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert(health.version === '1.6.146' && health.version_label === 'v1.6.146 · 客户云存储恢复版', 'public application health version should report v1.6.146');
