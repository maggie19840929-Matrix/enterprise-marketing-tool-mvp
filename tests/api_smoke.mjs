...
assert(diagnosis.app_version === '1.6.58', `expected app_version 1.6.58, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.58'"), 'app should expose v1.6.58 internally/API-side');
...
assert(localDevServer.includes("'/about'") && localDevServer.includes("'/privacy'") && localDevServer.includes("customerInfoPaths.has(pathname)"), 'local dev server should mirror customer info-page SPA rewrites');
...
assert(indexHtml.includes('/app.js?v=1.6.58') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.58'), 'customer page should cache-bust current v1.6.58 assets');
...
assert(indexHtml.includes('南京尚下联信息科技有限公司') && indexHtml.includes('contact@fpmatrix.cn'), 'customer info pages should expose the service entity and complaint email');
assert(indexHtml.includes('本地存储与云端同步') && indexHtml.includes('第三方服务与模型调用') && indexHtml.includes('查阅、复制、更正、补充、删除'), 'privacy policy should cover storage, model calls and data-subject rights');
assert(indexHtml.includes('AI 内容说明') && indexHtml.includes('禁止行为') && indexHtml.includes('内容效果和责任限制') && indexHtml.includes('不承诺固定流量、咨询量、成交量或商业结果') && indexHtml.includes('投诉与争议处理'), 'terms should cover AI content, prohibited behavior, effect disclaimer and complaint handling');
