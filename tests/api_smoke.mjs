...
assert(apiSource.includes("format === 'json'") && apiSource.includes('content_base64: Buffer.from(content.bytes).toString') && appJs.includes('function hydrateGenerationOutputMedia') && appJs.includes('data-generation-media-asset-id') && appJs.includes('window.atob(String(payload.content_base64') && appJs.includes('downloadGenerationTaskAsset'), 'generation task cards should hydrate and download separately stored image output through an authenticated JSON transport without blocking task-list rendering');
