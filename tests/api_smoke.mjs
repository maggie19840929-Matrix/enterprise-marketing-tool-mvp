...
const INTERNAL_ACCESS_TOKEN = 'smoke-internal-token-1.6.118';
const BACKGROUND_GENERATION_TOKEN = 'smoke-background-token-1.6.118';
const FEISHU_INBOUND_TOKEN = 'smoke-feishu-inbound-token-1.6.118';
...
process.env.METERING_HASH_SECRET = 'smoke-metering-secret-v1.6.118-not-production';
...
assert(diagnosis.app_version === '1.6.118', `expected app_version 1.6.118, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.118'") && appJs.includes("v1.6.118 · 多类型素材生成稳定版"), 'app should expose the v1.6.118 multi-type generation stability release');
...
assert(indexHtml.includes('/app.js?v=1.6.118') && indexHtml.includes('/styles.css?v=1.6.118') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.118'), 'customer page should cache-bust the v1.6.118 multi-type generation release while preserving the first-paint fix');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.118') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.118') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.118') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert([aboutHtml, privacyHtml, termsHtml, contactHtml].every((html) => html.includes('fp-matrix-lockup') && html.includes('企业第一方增长智能') && html.includes('/fp-matrix-elephant.svg?v=1.6.99') && html.includes('/fp-matrix-favicon.svg?v=1.6.99')), 'all independent customer information pages should use the same finalized FP Matrix identity');
...
assert(health.version === '1.6.118' && health.version_label === 'v1.6.118 · 多类型素材生成稳定版', 'health should expose the v1.6.118 multi-type generation stability release');
...
assert(health.providers?.openai === false && health.providers?.image_model === 'gpt-image-1', 'health should expose non-secret image provider readiness and model evidence');
