...
assert(diagnosis.app_version === '1.6.33', `expected app_version 1.6.33, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.33'"), 'app should expose v1.6.33 internally/API-side');
assert(appJs.includes('function isAnbiaoCustomerProject()') && appJs.includes('/安标检测|安标|医疗器械注册|医疗器械送检|注册送检/'), 'anbiao customer feedback panel should use explicit anbiao/medical-device project gate');
assert(!appJs.includes('/安检|安标|轻工业|医疗器械|检测/'), 'anbiao customer data panel must not match light-industry or generic detection projects');
