...
assert(diagnosis.app_version === '1.5.5.1', `expected app_version 1.5.5.1, got ${diagnosis.app_version}`);
...
assert(appJs.includes('>查看依据</button>') && appJs.includes('evidenceButton'), 'customer input evidence should be a compact button in the top key conclusion workbench');
assert(appJs.includes('>开始诊断</button>') && appJs.includes('>看计划</button>') && appJs.includes('>回填/复盘</button>'), 'workbench buttons should stay compact but understandable');
...
assert(indexHtml.includes('v1.5.5.1 · 客户理解修补版'), 'index should show v1.5.5.1 label');
assert(indexHtml.includes('先诊断，再发布，最后用数据复盘。'), 'hero should include a short workflow hint');
