...
assert(diagnosis.app_version === '1.7.3', `expected app_version 1.7.3, got ${diagnosis.app_version}`);
...
assert(appJs.includes('openClientEvidence') && appJs.includes('客户档案') && appJs.includes('系统判断') && appJs.includes('内容复盘依据'), 'review evidence should be reachable without exposing K/V/R labels');
assert(appJs.includes('查看复盘数据') && appJs.includes('重新填写'), 'internal review entry should be secondary after customer result');
...
assert(indexHtml.includes('v1.7.3 · 战情室视觉基线恢复版'), 'index should show v1.7.3 label');
...
assert(indexHtml.includes('只填发布链接、曝光互动和咨询数'), 'feedback section should keep simple customer-facing copy');
...
assert(indexHtml.includes('<h2>内容数据回填</h2>') && !indexHtml.includes('<h2>反馈回填</h2>'), 'feedback title should stay as content data refill');
assert(indexHtml.includes('曝光｜查看') && indexHtml.includes('互动｜点赞') && indexHtml.includes('互动｜评论') && indexHtml.includes('互动｜收藏/分享') && indexHtml.includes('转化｜私信/咨询'), 'feedback fields should be grouped as growth judgment signals');
