...
assert(diagnosis.app_version === '1.6.149', `public diagnosis should return app_version 1.6.149, got ${diagnosis.app_version}`);
...
assert(publicCommercialPlansData.pro_invite_only === false && publicCommercialPlansData.plans.find((plan) => plan.code === 'pro')?.public_sales === true, 'Pro should be a public standard plan while enterprise integrations remain evaluated separately');
...
const publicProOrder = await handler(request('POST', 'billing/orders', {
  plan_code: 'pro',
  billing_interval: 'month',
  idempotency_key: 'billing-pro-request-0001',
}, { headers: { cookie: billingAccountCookie } }));
const publicProOrderData = await publicProOrder.json();
assert(publicProOrder.status === 201 && publicProOrderData.order?.amount_cny === 899, 'signed-in customer should be able to create the standard Pro monthly order at the server-owned price');
...
assert(appJs.includes("const APP_VERSION = '1.6.149'") && appJs.includes("v1.6.149 · 对标洞察操作引导版"), 'application should expose the reviewed v1.6.149 benchmark workflow guidance release');
assert(appJs.includes('账号已保存，请继续完成第 3 步') && appJs.includes('contentForm?.scrollIntoView') && appJs.includes('profilePicker.value = result.profile?.benchmark_profile_id') && appJs.includes("analyzeButton.disabled = benchmarkWorkbenchState.loading || running || !activeProfiles.length || !readyContents.length"), 'benchmark workflow should confirm account saves, lead operators to representative content, and keep analysis disabled until evidence is ready');
assert(appJs.includes('const runBenchmarkAction') && appJs.includes("'代表内容未保存'") && appJs.includes("benchmark_profile_id: '请先选择这条内容所属的对标账号'") && appJs.includes("addEventListener('invalid'"), 'benchmark forms should keep validation and API errors visible instead of failing with a transient toast');
assert(appJs.includes('function customerStateProjectName') && appJs.includes('rawName.split(/[，,。；;：:\\n\\r]/)') && appJs.includes('characters.length > 20'), 'workspace project names should use a concise first phrase and cap long unpunctuated labels');
assert(warRoomCss.includes('grid-template-columns:minmax(0,220px) minmax(0,1fr)!important') && warRoomCss.includes('max-width:220px!important'), 'workspace project context should have a bounded desktop column so progress tabs cannot overlap it');
assert(appJs.includes('function continueCustomerSavedProject') && appJs.includes('customerResumeCandidateState || loadCustomerTrialState({allowDedicatedFallback: true})') && appJs.includes('renderCustomerGeneratedState(saved, {step: customerDefaultStep(saved), focus: true})'), 'resume action should restore the exact displayed project state instead of re-reading a filtered empty state');
...
assert(plansHtml.includes('联系评估企业定制方案') && plansHtml.includes('飞书系统打通') && plansJs.includes('data-plan-checkout="${esc(plan.code)}"'), 'Pro should use the standard checkout while contact evaluation is reserved for enterprise integrations');
...
assert(indexHtml.match(/id="customerProgressStrip"/g)?.length === 1 && indexHtml.includes('id="customerWorkspaceNav"') && appJs.includes('renderCustomerWorkspaceContext'), 'generated customer work should use one persistent progress navigator with project context');
assert(appJs.includes('CUSTOMER_RESUME_SESSION_PREFIX') && appJs.includes('rememberCustomerResumeDecision(saved)') && appJs.includes('renderCustomerResumeBanner(saved)'), 'continuing a local project should dismiss the resume prompt for the current browser session');
assert(indexHtml.includes('class="customer-strategy-details"') === false && appJs.includes('class="customer-strategy-details"') && appJs.includes('class="customer-strategy-summary"'), 'customer strategy should render a short summary and progressively disclose supporting detail');
assert(publicAccountMenuJs.includes('fetchWithTimeout') && publicAccountMenuJs.includes('trigger.disabled = false') && publicAccountMenuJs.includes('state.loading || state.error'), 'the account entry should remain actionable while session state is slow or temporarily unavailable');
assert(appJs.includes('if (accountBtn && !window.publicAccountMenu)') && publicAccountMenuJs.includes('window.publicAccountMenu = { refresh: load, close }'), 'the shared account menu should be the only owner of the navigation account label');
assert(netlifyToml.includes('max-age=31536000, immutable') && netlifyToml.includes('for = "/app.js"') && netlifyToml.includes('for = "/war-room-v1.6.1.css"'), 'versioned static assets should use long-lived immutable caching instead of downloading again on every refresh');
...
assert(indexHtml.includes('class="customer-inline-link customer-result-primary"') && warRoomCss.includes('body.customer-mode a.customer-result-primary{') && warRoomCss.includes('background:#111827!important;\n  color:#fff!important') && warRoomCss.includes('font-weight:760!important'), 'the result-page primary action should remain visually distinct from neutral inline links');
...
assert(apiSource.includes('cachedCloudStore') && apiSource.includes('__store_probe__') && apiSource.includes('getBlobsStore(CLOUD_STATE_STORE)') && apiSource.includes('getBlobsStore({ name: CLOUD_STATE_STORE, siteID, token'), 'cloud store should prefer the runtime-injected site store before falling back to verified explicit credentials');
...
assert(apiSourceIncludes("from '@netlify/blobs'") && apiSourceIncludes('getBlobsStore(CLOUD_STATE_STORE)') && apiSourceIncludes('await explicitStore.get'), 'cloud storage should statically declare its dependency, prefer the deployment-scoped runtime context without requiring strong consistency, and verify any explicit fallback credentials');
...
assert(indexHtml.includes('/app.js?v=1.6.149') && indexHtml.includes('/styles.css?v=1.6.149') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.149'), 'public customer page should use the v1.6.149 cache-busted asset references');
...
assert(stylesCss.includes('v1.6.145 internal benchmark fields') && stylesCss.includes('body.internal-mode.benchmark-insights-mode .benchmark-form input:-webkit-autofill') && stylesCss.includes('body.internal-mode .benchmark-block textarea') && stylesCss.includes('-webkit-text-fill-color:#f7f8f8!important'), 'internal benchmark account fields should keep typed and autofilled text readable without changing customer styles');
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.149') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.149') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.149') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert(plansHtml.includes('war-room-v1.6.1.css?v=1.6.148') && plansHtml.includes('/public-account-menu.js?v=1.6.148') && [methodHtml, aboutHtml, contactHtml, privacyHtml, termsHtml, inviteHtml].every((html) => html.includes('war-room-v1.6.1.css?v=1.6.138') && html.includes('/public-account-menu.js?v=1.6.138') && !html.includes('class="customer-account-nav" href="/?account=login"')), 'payment plan page should retain its independent current assets while unchanged public pages retain the shared session-aware account menu');
...
assert(health.version === '1.6.149' && health.version_label === 'v1.6.149 · 对标洞察操作引导版', 'public application health version should report v1.6.149');
