...
const INTERNAL_ACCESS_TOKEN = 'smoke-internal-token-1.6.98';
const FEISHU_INBOUND_TOKEN = 'smoke-feishu-inbound-token-1.6.98';
process.env.INTERNAL_ACCESS_TOKEN = INTERNAL_ACCESS_TOKEN;
process.env.METERING_HASH_SECRET = 'smoke-metering-secret-v1.6.98-not-production';
...
assert(diagnosis.app_version === '1.6.98', `expected app_version 1.6.98, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.98'") && appJs.includes("v1.6.98 · FP Matrix 品牌标识定稿版"), 'app should expose the v1.6.98 official brand identity release internally/API-side');
...
assert(indexHtml.includes('/app.js?v=1.6.98') && indexHtml.includes('/styles.css?v=1.6.98') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.98'), 'customer page should cache-bust the v1.6.98 official brand identity release while preserving the first-paint fix');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.98') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.98') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
...
assert(warRoomCss.includes('v1.6.98 FP Matrix official brand lockup') && warRoomCss.includes('color:#1a1c24') && warRoomCss.includes('color:#4d4d4d') && warRoomCss.includes('letter-spacing:.18em'), 'official lockup should use the approved brand colors, medium MATRIX weight, and tightened Chinese descriptor');
assert([aboutHtml, privacyHtml, termsHtml, contactHtml].every((html) => html.includes('fp-matrix-lockup') && html.includes('企业第一方增长智能') && html.includes('/fp-matrix-elephant.svg?v=1.6.98') && html.includes('/fp-matrix-favicon.svg?v=1.6.98')), 'all independent customer information pages should use the same finalized FP Matrix identity');
...
assert(health.version === '1.6.98' && health.version_label === 'v1.6.98 · FP Matrix 品牌标识定稿版', 'health should expose the v1.6.98 official brand identity release');
