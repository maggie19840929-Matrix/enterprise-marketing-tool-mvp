...
assert(diagnosis.app_version === '1.6.150', `public diagnosis should return app_version 1.6.150, got ${diagnosis.app_version}`);
...
assert(appJs.includes("const APP_VERSION = '1.6.150'") && appJs.includes("v1.6.150 · 私密链接恢复修复版"), 'application should expose the reviewed v1.6.150 private-link recovery release');
assert(appJs.includes("console.error('[customer_project_restore_failed]'") && appJs.includes("source: shareToken ? 'share_link' : 'customer_state'"), 'customer project recovery failures should be observable without exposing customer payloads');
...
assert(indexHtml.includes('/app.js?v=1.6.150') && indexHtml.includes('/styles.css?v=1.6.150') && indexHtml.includes('/war-room-v1.6.1.css?v=1.6.150'), 'public customer page should use the v1.6.150 cache-busted asset references');
...
assert(indexHtml.includes('fp-matrix-lockup') && indexHtml.includes('fp-matrix-elephant.svg?v=1.6.150') && indexHtml.includes('/fp-matrix-favicon.svg?v=1.6.150') && indexHtml.includes('<strong>FP</strong><em>MATRIX</em>') && indexHtml.includes('企业第一方增长智能'), 'customer page should expose the official FP Matrix lockup and dedicated favicon');
assert(indexHtml.includes('customer-product-lockup') && indexHtml.includes('/huoke-compass-mark.svg?v=1.6.150') && indexHtml.includes('获客<span>罗盘</span>') && indexHtml.includes('by FP Matrix'), 'customer hero should expose the Huoke Compass product lockup below the parent brand');
...
assert(health.version === '1.6.150' && health.version_label === 'v1.6.150 · 私密链接恢复修复版', 'public application health version should report v1.6.150');
