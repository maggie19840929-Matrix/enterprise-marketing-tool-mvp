...
assert(apiSource.includes("format === 'json'") && apiSource.includes('Binary payloads must bypass customer text sanitization') && apiSource.includes('content_base64: Buffer.from(content.bytes).toString') && appJs.includes('function hydrateGenerationOutputMedia') && appJs.includes('data-generation-media-asset-id') && appJs.includes('window.atob(String(payload.content_base64') && appJs.includes('await node.decode()') && appJs.includes('downloadGenerationTaskAsset'), 'generation task cards should hydrate and download separately stored image output through an authenticated unsanitized JSON transport without blocking task-list rendering');
